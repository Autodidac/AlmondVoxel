...
TEST_CASE(classic_heightfield_respects_surface_and_bedrock_layers) {
    ...
    config.surface_material = material_index{42};
    config.filler_material = material_index{43};
    config.subsurface_material = material_index{44};
    config.bedrock_material = material_index{45};
    config.air_material = material_index{1};
    ...
    for (std::uint32_t z = 0; z < extent.z; ++z) {
        ...
        if (z < config.bedrock_layers) {
            ...
            CHECK_MESSAGE(mat == config.filler_material, "filler material mismatch at z=" << z);
        } else {
            CHECK_MESSAGE(id == config.subsurface_voxel, "subsurface block mismatch at z=" << z);
            CHECK_MESSAGE(mat == config.subsurface_material, "subsurface material mismatch at z=" << z);
        }
    }
}
